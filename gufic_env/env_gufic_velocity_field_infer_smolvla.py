...
from gufic_env.flow_matching.model import VelocityRegressiveMLP, VelocityFMTransformer, VisionPoseObsEncoder, VisionDeltaPoseFMTransformer, VelocityFMTransformer, VelocityFMCondUnet1D, VisionPoseObsEncoderPoseSimpleV2, VisionPoseObsEncoderNoPCForVelocity
...
from gufic_env.flow_matching.infer_fm import load_one_demo, get_velocity_field, vd_body_to_dpd_dRd, recover_pose_from_delta, recover_desired_pose_from_abs 
from gufic_env.flow_matching.infer_smolvla import load_smolvla_velocity_field_infer
# ============================================================
...
class RobotEnv:
    def __init__(
        ...
        separation_vp=True,
        # ===== SmolVLA online infer related =====
        smolvla_policy_path=None,
        smolvla_dataset_repo_id="gufic_boltnut_pi0",
        smolvla_dataset_root="/media/zhou/Elements SE/VLA/boltnut3_pi0_lerobot_random_start",
        smolvla_vlm_model_name=None,
        smolvla_action_mode="pose",
        smolvla_language="insert the bolt into the hole",
        smolvla_policy_hz=None,
        smolvla_default_policy_hz=20.0,
        smolvla_contact_policy_hz=50.0,
        ...
        self.smolvla_policy_path = smolvla_policy_path
        self.smolvla_dataset_repo_id = smolvla_dataset_repo_id
        self.smolvla_dataset_root = smolvla_dataset_root
        self.smolvla_vlm_model_name = smolvla_vlm_model_name
        self.smolvla_action_mode = smolvla_action_mode
        self.smolvla_language = smolvla_language
        if smolvla_policy_hz is not None:
            smolvla_default_policy_hz = smolvla_policy_hz
        self.smolvla_default_policy_hz = float(smolvla_default_policy_hz)
        self.smolvla_contact_policy_hz = float(smolvla_contact_policy_hz)
        self.smolvla_contact_force_threshold = float(smolvla_contact_force_threshold)
        ...
        elif self.test_offline_cond and self.policy != "smolvla":
            default_demo_path = "/home/zhou/autolab/GUFIC_mujoco-main/bolt_vis_demo/bolt_demo_0000.npz"
            self.demo = load_one_demo(default_demo_path)
        self.fz = fz
        ...
        self.smolvla_default_decimation = max(
            1,
            int(round(1.0 / (max(self.smolvla_default_policy_hz, 1e-8) * self.dt))),
        )
        self.smolvla_contact_decimation = max(
            1,
            int(round(1.0 / (max(self.smolvla_contact_policy_hz, 1e-8) * self.dt))),
        )
        self.smolvla_default_actual_hz = 1.0 / (self.smolvla_default_decimation * self.dt)
        self.smolvla_contact_actual_hz = 1.0 / (self.smolvla_contact_decimation * self.dt)
        self.smolvla_cached_result = None
        self.smolvla_cached_Vd_star = np.zeros((6,), dtype=np.float32)
        self.smolvla_cached_dVd_star = np.zeros((6,), dtype=np.float32)
        self.smolvla_last_update_iter = -1
        self.smolvla_in_contact = False
        ...
        if self.use_learned_velocity_field and self.policy == "smolvla":
            self._load_smolvla_model()
        elif self.use_learned_velocity_field:
            self._load_fm_model()
    ...
    def _load_smolvla_model(self):
        self.smolvla_infer = load_smolvla_velocity_field_infer(
            ...
            device=self.fm_device,
            zeta_v=self.zeta_v,
            zeta_w=self.zeta_w,
            action_mode=self.smolvla_action_mode,
            vlm_model_name=self.smolvla_vlm_model_name,
        )
        self.smolvla_infer.reset()
        print(f"[SmolVLA] loaded policy from {self.smolvla_infer.policy_path}")
        print(f"[SmolVLA] dataset_repo_id = {self.smolvla_dataset_repo_id}")
        print(f"[SmolVLA] dataset_root = {self.smolvla_dataset_root}")
        print(f"[SmolVLA] VLM backbone = {self.smolvla_infer.vlm_model_name}")
        print(f"[SmolVLA] action_mode = {self.smolvla_action_mode}")
        print(
            f"[SmolVLA] non_contact_hz = {self.smolvla_default_policy_hz}, "
            f"dt = {self.dt}, decimation = {self.smolvla_default_decimation}, "
            f"actual_hz = {self.smolvla_default_actual_hz:.3f}"
        )
        print(
            f"[SmolVLA] contact_hz = {self.smolvla_contact_policy_hz}, "
            f"decimation = {self.smolvla_contact_decimation}, "
            f"actual_hz = {self.smolvla_contact_actual_hz:.3f}, "
            f"force_threshold = {self.smolvla_contact_force_threshold}"
        )
        print(
            f"[SmolVLA] reset_each_update = {self.smolvla_reset_each_update}, "
            f"ablation_mode = {self.smolvla_ablation_mode}"
        )

    def _reset_smolvla_cache(self):
        if not hasattr(self, "smolvla_cached_Vd_star"):
            return

        self.smolvla_cached_result = None
        self.smolvla_cached_Vd_star[:] = 0.0
        self.smolvla_cached_dVd_star[:] = 0.0
        self.smolvla_last_update_iter = -1
        self.smolvla_in_contact = False

        if self.smolvla_infer is not None:
            self.smolvla_infer.reset()
    ...
    def _load_fm_model(self):
        ...
        # ).to(self.fm_device)
        # self.obs_encoder = VisionDeltaPoseFMTransformer(
        #     state_dim=train_cfg["state_dim"],
        #     cond_dim=train_cfg["cond_dim"],
        #     guide_dim=train_cfg["guide_dim"],
        #     embed_dim=train_cfg["embed_dim"],
        ...
        #     nhead=8,
        #     dropout=0.1,
        #     sample_steps=getattr(train_cfg, "infer_steps", 10),
        # ).to(self.fm_device)
        obs_dim = int(getattr(train_cfg, "obs_dim", 128))

        self.obs_encoder = VisionPoseObsEncoderNoPCForVelocity(
            state_dim=train_cfg["state_dim"],
            cond_dim=train_cfg["cond_dim"],
            obs_dim=obs_dim,
            guide_dim=train_cfg["guide_dim"],
        ...
        self.pose_model = VelocityFMTransformer(
            x_dim=9,                 # [pd(3), Rd6d(6)]
        ...
        self.pose_model.load_state_dict(ckpt["pose_model"])
    ...
    def get_learned_velocity_field(self, p, R, t, Fe, point_cloud):
        if self.policy == "smolvla":
            if self.smolvla_infer is None:
                raise RuntimeError("SmolVLA infer is not loaded. Set model='smolvla' and use_learned_velocity_field=True.")

            p_now = np.asarray(p, dtype=np.float32).reshape(-1, 3)[-1]
            R_now = np.asarray(R, dtype=np.float32).reshape(-1, 3, 3)[-1]
            Fe_now = np.asarray(Fe, dtype=np.float32).reshape(-1)[:6]
            smolvla_contact = abs(float(Fe_now[2])) > self.smolvla_contact_force_threshold
            smolvla_decimation = (
                self.smolvla_contact_decimation
                if smolvla_contact
                else self.smolvla_default_decimation
            )
            smolvla_contact_changed = smolvla_contact != self.smolvla_in_contact

            update_smolvla = (
                self.smolvla_cached_result is None
                or smolvla_contact_changed
                or (self.iter - self.smolvla_last_update_iter) >= smolvla_decimation
            )

            if update_smolvla:
                wrist_image = self.resize_rgb(self.get_camera_rgb(self.cam_id))
                external_image = self.resize_rgb(self.get_external_rgb())
                # Image.fromarray(wrist_image).save("online_wrist.png")
                # Image.fromarray(external_image).save("online_external.png")
                if self.smolvla_reset_each_update:
                    self.smolvla_infer.reset()
                result = self.smolvla_infer.predict_velocity_field(
                    wrist_image=wrist_image,
                    external_image=external_image,
                    p=p_now,
                    R=R_now,
                    Fe=Fe_now,
                    ablation_mode=self.smolvla_ablation_mode,
                    dpd=self.dpd_t(self.data.time).reshape(3),
                    dRd=self.dRd_t(self.data.time).reshape(3, 3),
                )
                Vd_star = np.asarray(result["Vd_star"], dtype=np.float32).reshape(6)

                if self.smolvla_last_update_iter < 0:
                    dVd_star = np.zeros((6,), dtype=np.float32)
                else:
                    dt_update = (self.iter - self.smolvla_last_update_iter) * self.dt
                    dVd_star = (Vd_star - self.smolvla_cached_Vd_star) / max(dt_update, 1e-8)

                alpha = self.fm_lowpass_alpha
                dVd_star = alpha * dVd_star + (1.0 - alpha) * self.smolvla_cached_dVd_star

                self.smolvla_cached_result = result
                self.smolvla_cached_Vd_star = Vd_star.copy()
                self.smolvla_cached_dVd_star = dVd_star.copy()
                self.smolvla_last_update_iter = self.iter
                self.smolvla_in_contact = smolvla_contact
            else:
                result = self.smolvla_cached_result
                Vd_star = self.smolvla_cached_Vd_star.copy()
                dVd_star = self.smolvla_cached_dVd_star.copy()

            if smolvla_contact_changed or self.iter % max(1, self.smolvla_default_decimation * 5) == 0:
                pd_gt = self.pd_t(self.data.time).reshape(3)
                print("[SmolVLA] update:", update_smolvla)
                print("[SmolVLA] contact:", smolvla_contact, "decimation:", smolvla_decimation)
                print("[SmolVLA] p_now:", p_now)
                print("[SmolVLA] pd_pred:", result["pd"].reshape(3))
                print("[SmolVLA] pd_gt:", pd_gt)
                print("[SmolVLA] ||pd_pred - pd_gt||:", np.linalg.norm(result["pd"].reshape(3) - pd_gt))
                print("[SmolVLA] ||p_now - pd_pred||:", np.linalg.norm(p_now - result["pd"].reshape(3)))
                print("[SmolVLA] Vd_body:", result["Vd_body"])
                print("[SmolVLA] Vd_star:", Vd_star)
                print("[SmolVLA] action[:9]:", result["action"][:9])
            ...
            if self.use_cond:
                ...
                #     cond_hist=cond,
                # )
                nx, guide_feat = self.obs_encoder(
                    cond_pc,
                    x_now,
                    cond_hist=cond,
                )
                # 离线评估可以用随机初值；在线控制建议改成 zeros 或固定低通。
                z_pose = torch.randn(1, 1, 9, device=self.fm_device)
                ...
                for i in range(self.steps):
                    ...
                    z_pose = z_pose + u_pose * (1.0 / self.steps)

                desired_pose_pred_norm = (
                    z_pose.squeeze(0).detach().cpu().numpy().astype(np.float32)
                )  # [9]
                
                self._update_delta_pose_marker(z_pose, p_raw, R_raw)
    ...
    def geometric_unified_force_impedance_control(self):
        ...
        if self.test_offline_cond:
            if self.demo is None:
                raise RuntimeError(
                    "test_offline_cond=True requires a demo npz. "
                    "SmolVLA online inference does not use demo_path; set test_offline_cond=False."
                )
        ...
        # pointcloud is only needed by the FM visual transformer path.
        if self.policy == "smolvla":
            point_cloud = None
        elif self.iter % self.pointcloud_capture_every == 0:
            point_cloud = self.capture_point_cloud()
            if self.use_pc_color:
...
if __name__ == "__main__":
    ...
    #   mlp:
    # ============================================================
    model = 'smolvla' # smolvla, unet, transformer, mlp
    ...
    smolvla_policy_path = os.environ.get(
        "SMOLVLA_POLICY_PATH",
        (
    ...
    smolvla_vlm_model_name = os.environ.get(
    ...
    smolvla_dataset_root = os.environ.get(
        "SMOLVLA_DATASET_ROOT",
        "/media/zhou/Elements SE/VLA/boltnut_pi0_lerobot_20HZ",
    )
    ...
    RE = RobotEnv(
        ...
        test_offline_cond=False,
        visualize_delta_pose=True,
        smolvla_policy_path=smolvla_policy_path,
        smolvla_dataset_repo_id="gufic_boltnut_smolvla",
        smolvla_dataset_root=smolvla_dataset_root,
        smolvla_vlm_model_name=smolvla_vlm_model_name,
        smolvla_action_mode="full",
        smolvla_language="insert the bolt into the hole",
        smolvla_default_policy_hz=50.0,
        smolvla_contact_policy_hz=50.0,
        smolvla_contact_force_threshold=1.0,
        smolvla_reset_each_update=True,
        smolvla_ablation_mode=True,
