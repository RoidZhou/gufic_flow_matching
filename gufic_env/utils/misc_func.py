    ...
    if controller == "GIC":
        ...
        if task in ["regulation", "insertion"]:
            ...
            if sim_mode == "demonstration":
                ...
                zeta_v = 50
                zeta_w = 10
            elif sim_mode == "smlovla_position_infer":
                Kp = np.eye(3) * np.array([5000, 5000, 5000]) # infer
                KR = np.eye(3) * np.array([5000, 5000, 5000])
                Kd = np.eye(6) * np.array([500, 500, 500, 500, 500, 500])
                ...
                Kp = np.eye(3) * np.array([2000, 2000, 2000]) # infer
                KR = np.eye(3) * np.array([2000, 2000, 2000])
                Kd = np.eye(6) * np.array([500, 500, 500, 500, 500, 500])
